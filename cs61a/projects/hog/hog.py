...
def left_most(num, nth=1):
    string = str(num)
    if num < 10:
        string = '0' + string
    assert nth <= len(string), 'nth must be less than bits number'
    return int(string[nth-1])
...
def more_boar(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 4
    return left_most(player_score) < left_most(opponent_score) and left_most(player_score, 2) < left_most(opponent_score, 2)
    # END PROBLEM 4
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        ...
        if who == 0:
            ...
            is_more_bar = more_boar(score0, score1)
        else:
            score1 += take_turn(strategy1(score1, score0), score0, dice, goal)
            is_more_bar = more_boar(score1, score0)
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    # BEGIN PROBLEM 9
    max_scores = 0
    max_scores_dice_num = 0
    dice_num = 1
    while dice_num <= 10:
        averaged = make_averaged(roll_dice, trials_count)
        average_score = averaged(dice_num, dice)
        if average_score > max_scores:
            max_scores = average_score
            max_scores_dice_num = dice_num
        dice_num += 1
    return max_scores_dice_num
    # END PROBLEM 9
