...
def choose(paragraphs, select, k):
    ...
    return selected[k] if len(selected) - 1 >= k else ""
    # END PROBLEM 1
...
def about(topic):
    ...
    def select(paragraph):
        ...
        return len([True for t in topic if t in words]) > 0
    return select
    # END PROBLEM 2
...
def accuracy(typed, reference):
    ...
    correctCnt = len([True for tw, rw in list(zip(typed_words, reference_words)) if tw == rw])
    return (correctCnt / len(typed_words)) * 100
    # END PROBLEM 3
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    for w in valid_words:
        ...
        if word == None:
            ...
            word = w if diff < diff_function(typed_word, word, limit) else word
        
    return word if word != None else typed_word
    # END PROBLEM 5
...
def sphinx_switches(start, goal, limit):
    ...
    cnt = 1 if start[0] != goal[0] else 0
    return cnt + sphinx_switches(start[1:], goal[1:], limit-1 if cnt == 1 else limit)
    # END PROBLEM 6
...
def report_progress(typed, prompt, user_id, send):
    ...
    # BEGIN PROBLEM 8
    correctCnt = 0
    for t, p in list(zip(typed, prompt)):
        if t == p:
            correctCnt += 1
        else:
            break
    progress = correctCnt / len(prompt)
