...
def num_eights(x):
    ...
    if x < 10:
        ...
        return num_eight(x % 10) + num_eights(x//10)
    ...
    def pp(i, dir, p):
        if i == n:
            ...
            if i % 8 == 0 or num_eights(i) > 0:
                return pp(i+1, -1 * dir, p + (-1 * dir))
            else:
                return pp(i+1, dir, p + dir)
    return pp(1, 1, 1)
...
def count_coins(change):
    ...
    def count(change, current):
        if change == 0:
            ...
            return 0
        return count(change-current, current) + count(change, get_next_coin(current))
